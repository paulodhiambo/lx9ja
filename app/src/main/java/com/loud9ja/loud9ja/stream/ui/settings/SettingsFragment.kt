...
import com.loud9ja.loud9ja.databinding.FragmentSettingsBinding
import com.loud9ja.loud9ja.BuildConfig
import com.loud9ja.loud9ja.stream.ui.meeting.MeetingViewMode
import com.loud9ja.loud9ja.stream.util.ENV_PROD
import com.loud9ja.loud9ja.stream.util.ENV_QA
...
class SettingsFragment : Fragment() {
    ...
    companion object {
        ...
        private val MEETING_MODES =
            MeetingViewMode::class.nestedClasses.mapNotNull { it.simpleName }.toTypedArray()

        private val LOG_LEVELS_100MS =
            HMSLogger.LogLevel.values().map { it.toString() }.toTypedArray()
            ...
            addTextChangedListener { text ->
                if (text.toString().isEmpty()) {
                    ...
                    if (value < minValue || value > maxValue) {
                        container.error =
                            "$name value should be in range [$minValue, $maxValue] inclusive"
                    } else {
                        container.error = null
                        saveOnValid(value)
                    }
